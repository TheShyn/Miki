...
import { getProduct1, updateProduct } from "@/instance/Products";
...
export default function UpdateProduct({ }: Props) {
    ...
    const methods = useForm<any>({
        ...
        defaultValues: {
            ...
            dynamicForm: product?.storage?.map((item: any) => {
                return {
                    size: item.size,
    ...
    const onSubmit = (data: any) => {
        ...
        if (data) {
            ...
            console.log(dataUp);
            dipatch(updateProduct({ id: product._id, data: dataUp }))

        }
    }
    useEffect(() => {
        dipatch(getProduct1(`${id}`))
